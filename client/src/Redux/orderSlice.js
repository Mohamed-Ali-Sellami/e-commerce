...
export const getorder = createAsyncThunk("/getorder", async () => {
  try {
    let resultat = axios.get("http://localhost:5500/order/all");
...
export const addorder = createAsyncThunk("/addorder", async (neworder) => {
  try {
    let resultat = axios.post("http://localhost:5500/order/add", neworder);
    return await resultat;
  } catch (error) {
    console.log(error);
  }
...
export const deleteorder = createAsyncThunk("/deleteorder", async (id) => {
  try {
    let resultat = axios.delete(`http://localhost:5500/order/${id}`);
    return await resultat;
  } catch (error) {
    console.log(error);
  }
...
export const updateorder = createAsyncThunk(
  ...
  async ({ id, uporder }) => {
    try {
      let resultat = axios.put(`http://localhost:5500/order/${id}`, uporder);
      return await resultat;
    } catch (error) {
      console.log(error);
    }
